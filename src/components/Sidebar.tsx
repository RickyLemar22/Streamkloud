...
import { Home, Music, Diamond, Layers, Plus, ListMusic, Settings } from 'lucide-react';
...
export function Sidebar() {
  ...
  const handleCreatePlaylist = () => {
    ...
    if (name) {
      createPlaylist(name).catch((error) => alert(error.message));
    }
  };
    ...
    <>
      ...
      <div className="hidden lg:flex flex-col h-full bg-black text-zinc-400 w-72 p-8 gap-y-12 border-r border-zinc-900 shrink-0">
        ...
        <nav className="space-y-6 flex-1 overflow-y-auto no-scrollbar">
          ...
          {isAdmin && (
            <div className="space-y-2 pt-6">
              <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest px-4 mb-4">Admin</p>
              <Link 
                to="/admin" 
                className={cn(
